...
(function($) {
	$.extend({
		progressBar: new function() {
			...
			this.defaults = {
				steps			: 100,											// steps taken to reach target
				step_duration	: 20,											
				max				: 100,											// Upon 100% i'd assume, but configurable
				showText		: true,											// show text with percentage in next to the progressbar? - default : true
				textFormat		: 'porcentagem',									// Or otherwise, set to 'fraction'
				...
				boxImage		: '/media/images/progressbar.gif',						// boxImage : image around the progress bar
				barImage		: {
			...
			this.construct = function(arg1, arg2) {
				...
				return this.each(function(child) {
					...
					function getText(config) {
						if (config.showText) {
							if (config.textFormat == 'porcentagem') {
								return " " + Math.round(config.running_value) + "%";
							} else if (config.textFormat == 'fracao') {
